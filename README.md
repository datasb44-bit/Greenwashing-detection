...
2. Install dependencies:
   ...
   ```
3. Place your `amazon.csv` in the root directory and update `CSV_PATH` in the script if needed (default is `/content/amazon.csv` for Google Colab).
...
A frozen, preprocessed feature dataset (`amazon_greenwashing_features.csv`) is included in this repository. To skip the full pipeline (including image downloading) and reproduce results exactly, place this file in your output directory — the script will detect and load it automatically.

All pipeline configuration parameters are saved to `run_config.json` on each run.
